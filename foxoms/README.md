...
资源可以记录当前默认单位成本；每次投入关系独立保存数量、期间和归集成本，因此调整默认
成本不会改写历史业务成本。归集成本表示资源消耗在具体业务上的成本，不等同于工资、采购
或付款等现金支出。
...
捷径。Action 服务还会校验受管经营方、中标结果、唯一签约路径、资源投入日期、数量和成本、
...
- 总览将已开票、已回款和人员、软件、硬件直接成本放在同一视图，并展示“已开票减
  直接归集成本”得到的当前贡献；
- 商务拓展沿商机、招标、投标、商务约定展开，并将“暂无后续”显示为合法状态；
- 履约经营以框架协议或项目合同为单元汇总开票、回款、成本和当前贡献，再展开订单、
  项目/任务及成本构成；
- 开票回款按合同或订单汇总发票余额，并展开一笔回款核销多张发票的事实；
- 资源资产分别展示人员、软件、硬件的默认单位成本、实际平均单位成本、历史归集成本
  和投入去向，以及知识资产的业务角色。

“当前贡献”不是会计利润：模型尚未包含收入确认、采购成本、期间费用和税务调整，因此
页面只在可追溯范围内比较已开票金额与直接资源归集成本。项目合同的发票也不会在缺少
分摊依据时摊到单个项目/任务上，任务详情只展示自身直接成本。
