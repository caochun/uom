"""Serve the FoxOMS enterprise operations workbench and OAG Agent API."""
...
from foxoms.app.agent_runtime import OagAgentRuntime
from uom.workspace import ChangeValidationError


ROOT = Path(__file__).resolve().parents[2]
STATIC_ROOT = Path(__file__).resolve().parent / "static"
AGENT = OagAgentRuntime(ROOT, domain_dir=ROOT / "foxoms")


class FoxOmsHandler(BaseHTTPRequestHandler):
    server_version = "FoxOMS/0.1"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path == "/api/changes/preview":
                ...
                self._json(AGENT.call_domain("apply_changes", operations=body.get("operations")))
...
def main() -> int:
    ...
    server = ThreadingHTTPServer((args.host, args.port), FoxOmsHandler)
    print(f"FoxOMS running at http://{args.host}:{args.port}")
    try:
