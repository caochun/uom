...
class LeasingActionService(ModelActionService):
    ...
    def _enrich_approval_start(
        ...
        reviewed = self.workspace.repository.query_by_id("Object", reviewed_id)
        ...
        for relation in self.workspace.repository.query("Relation"):
            if (
                ...
                approval = self.workspace.repository.query_by_id(
    ...
    def _apply_reviewed_object_decision(
        ...
        reviewed = self.workspace.repository.query_by_id("Object", reviewed_ids[0])
        if not reviewed or reviewed.get("type") != "lease_plan":
            return
    ...
    def _validate_contract_signing(self, inputs: dict[str, Any]) -> None:
        plan = self.workspace.repository.query_by_id("Object", inputs["lease_plan_id"])
    ...
    def _validate_schedule_replacement(self, inputs: dict[str, Any]) -> None:
        ...
        schedules = {
            schedule_id: self.workspace.repository.query_by_id("Object", schedule_id)
    ...
    def _enrich_payment_allocation(
        ...
        payment = self.workspace.repository.query_by_id("Object", inputs["payment_id"])
        target = self.workspace.repository.query_by_id("Object", inputs["target_id"])
    ...
    def _enrich_contract_settlement(
        ...
        for relation in self.workspace.repository.query("Relation"):
            if relation.get("type") != "contains" or relation.get("from") != contract_id:
                continue
            participant = self.workspace.repository.query_by_id("Object", relation.get("to"))
        ...
        for schedule_id in self._contract_schedule_ids(contract_id):
            schedule = self.workspace.repository.query_by_id("Object", schedule_id)
    ...
    def _contract_credit_position(self, contract_id: str) -> tuple[str, float, str]:
        objects = {
            item["id"]: item
            for item in self.workspace.repository.query("Object")
